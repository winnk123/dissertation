...
class RLRunner(object):
    ...
    def output_goal(self, temp_map, temp_costmap, temp_pose, temp_merge_map):
        ...
        RobotData['global_goal'] = nn.Sigmoid()(eval_action)[0]
             ...
             # 将 global_goal 转换为栅格地图坐标
        #goal_x = int(RobotData['global_goal'][0] * RobotData['width'])
        #goal_y = int(RobotData['global_goal'][1] * RobotData['height'])

# 检查 global_goal 是否在地图范围内
        #if goal_x < 0 or goal_x >= RobotData['width'] or goal_y < 0 or goal_y >= RobotData['height']:
         #print("Global goal is outside the map bounds. Adjusting...")
         #goal_x = min(max(goal_x, 0), RobotData['width'] - 1)
         #goal_y = min(max(goal_y, 0), RobotData['height'] - 1)

# 筛选机制：检查 global_goal 是否落在自由空间
        #if merge_gridmap[goal_y, goal_x] != 0:  # 假设 0 表示自由空间，100 表示障碍物
          #print("Global goal in obstacle or unknown region. Adjusting...")
    # 寻找距离 global_goal 最近的自由空间点
         # found_free_space = False
          #for dx in range(-1, 2):
          #  for dy in range(-1, 2):
        #  nx, ny = goal_x + dx, goal_y + dy
            # 检查 nx 和 ny 是否在地图范围内
              #if 0 <= nx < RobotData['width'] and 0 <= ny < RobotData['height']:
                #if merge_gridmap[ny, nx] == 0:  # 确认该位置为自由空间
                   # goal_x, goal_y = nx, ny
...
# 如果周围没有自由空间，回退到当前位置
        #if merge_gridmap[goal_y, goal_x] != 0:
         # print("No valid global goal found, fallback to agent's current position.")
         # goal_x, goal_y = agent_pos[0], agent_pos[1]
